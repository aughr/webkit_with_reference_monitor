...
EncodedJSValue JSC_HOST_CALL booleanProtoFuncToString(ExecState* exec)
{
    ...
    if (asBooleanObject(thisValue)->internalValue() == jsBoolean(false))
        return JSValue::encode(jsNontrivialString(exec, "false"), exec, thisValue.isTainted());

    ASSERT(asBooleanObject(thisValue)->internalValue() == jsBoolean(true));
    return JSValue::encode(jsNontrivialString(exec, "true"), exec, thisValue.isTainted());
}
...
EncodedJSValue JSC_HOST_CALL booleanProtoFuncValueOf(ExecState* exec)
{
    ...
    return JSValue::encode(asBooleanObject(thisValue)->internalValue(), exec, thisValue.isTainted());
}
